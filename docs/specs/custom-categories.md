...
Nothing in the UI writes `categoryId` yet on a *newly created* transaction/budget/bill (that's stages 3-4) — every read path already tolerates that via the same name+type fallback the backfill itself uses, so there's no gap in between.
