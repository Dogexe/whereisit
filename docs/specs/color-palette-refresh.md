...
1. **"Purple" accent is relabeled "Blue"** and its hue shifts from violet
   toward the reference image's cleaner blue. The **stored preference
   value stays the literal string `"purple"`** (`state.accentColor`,
   `theme.js`'s `ACCENT` object key, `settings.js`'s radio `value="purple"`)
   — only its display label and hex values change. This avoids a data
   migration for any user who already has `"purple"` saved; per this
   project's standing rule, schema/data-format changes need an explicit
   ask, and none is needed here since the stored value is unchanged.
   - `i18n.js:52`: `accentColorPurpleOpt` → Thai "น้ำเงิน", English "Blue".
2. **"Coral" accent shifts warm — orange/amber — away from its current
   red-leaning hue.** Explicit reason: WI-016 makes expense amounts red
...
- Settings > Appearance > Accent color: two options remain, now labeled
  Coral (warmer/orange-shifted) and Blue (was Purple, now genuinely blue
  rather than violet). Selecting either updates `--color-accent`,
...
1. Toggle Accent color between Coral and Blue; confirm the hero balance
...
3. Measure and record actual contrast ratios (not eyeballed) for every
   ...
   accent/coral/blue base with white text (≥4.5:1), income-700/expense-700
